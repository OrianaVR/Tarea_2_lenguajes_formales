...
NUEVOS_NO_TERMINALES = list("ZYXWVUTRQPONMLKJIHGFEDCB")
indice_nuevo_nt = 0
...
def leer_linea_no_vacia():
    """Lee líneas de stdin, saltándose líneas vacías o solo espacios."""
    while True:
...
def formatear_gramatica(producciones, orden_original_nts):
    ...
    nuevos = [nt for nt in producciones.keys() if nt not in orden_original_nts]  
    ...
    for nt in orden_salida:
        if nt in producciones:
            ...
            if prods:
                salida.append(f"{nt} -> {' '.join(prods)}")
    return "\n".join(salida)


def eliminar_recursividad_directa(A, producciones):
    """Aplica la regla A -> beta A', A' -> alpha A' | e."""
...
def main():
    ...
    if n_casos_linea is None:
        return

    try:
        n_casos = int(n_casos_linea)
    except ValueError:
        return
